...
character(50)   :: outFileCode = '_PLG2_tPA01_along_Q2.dat'
...
!write(cbindfile,'(57a)') 'numbind_tPA425_PLG2_tPA01_along_Q2.dat'
!write(cindfile,'(57a)') 'numindbind_tPA425_PLG2_tPA01_along_Q2.dat'
!write(bind1file,'(57a)') 'bind_tPA425_PLG2_tPA01_along_Q2.dat'
            ...
            if (r.le.(1-q)) then   !i.e. if we stay on current edge
               ...
               if(bind(j).lt.t.and.bind(j).gt.0.and.degrade(V(1,j)).eq.0.and.t_wait(j).le.t) then
                ...
                if(r2.gt.(t-bind(j))/tstep) then   !if r2 is such that movement happened before binding, move the molecule
                 ...
                 if(t_wait(j)>t) then   !adjusted 9/15/17 to account for waiting time. if molecule has a waiting time>t, restrict its diffusion to be away from or along (not into) the clot
                 ...
                 else   !if there's no waiting time, or the waiting time is less than the current time, the molecule can move as normal
