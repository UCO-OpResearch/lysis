        ...
        do
            ...
            do j = 1, M
                ...
                if (V(2, j) == 1 .and. t_degrade(V(1, j)) < t) then
                    ...
                    !!                  That is, the molecule will be restricted in its movement and
                    !!                  binding until it would have unbound (if the fiber hadn't degraded first)
                    !! t_wait(j)=t_leave(j)
                    ...
                    if (r .le. (1 - q)) then   !i.e. if we stay on current edge
                        ...
                        if (bind(j) .lt. t .and. bind(j) .gt. 0 .and. t_degrade(V(1, j)) >= t) then
                            ...
                            if (t_wait(j) .le. t) then
                                ...
                                if (colr2 == 0 .or. colr2 == 1) then
                                    write (*, *) 'PROBLEM: colr2 should not equal 0 since CDFtPA goes between 0 and 1 exactly', &
                                        colr2
                                end if
                                ...
                                if (verbose) then
                                    ...
                                    write (*, '(A,I10,A,I5,A,F8.5,A,F5.4)') '-> ts ', count - 1, &
                                        ' -> m ', j - 1, ' leaving time set to ', t_leave(j), &
                                        '; using r = ', r3
                                    write (*, '(A, F8.5, A, F8.5)') ' interpolated between ', &
                                        tsec1(colr2), ' and ', tsec1(colr2 - 1)
                                end if
                        ...
                        if (bind(j) .lt. t .and. bind(j) .gt. 0 .and. t_degrade(V(1, j)) >= t .and. t_wait(j) .le. t) then
                            ...
                            if (r2 .gt. (t - bind(j))/tstep) then   !if r2 is such that movement happened before binding, move the molecule
                                ...
                                if (verbose) then
                                    ...
                                    write (*, '(F8.5, A, F8.5, A, F8.5)') ttPA, &
                                        '; interpolated between ', tsec1(colr2), ' and ', tsec1(colr2 - 1)
                                    !write (*,*) t, ttPA, tstep
