program macrolysis
    ...
    !! BRAD 2026-05-12: - Embed git build provenance via the version_stamp module
    !!                    (regenerated by the Makefile from `git rev-parse HEAD` and
    !!                    `git diff` of src/fortran/ each build)
    !!                  - `--version` flag prints "<tag> <commit> <clean|dirty> <compiler>"
    !!                    and exits, for preflight checks by the Python wrapper
    !!                    before running.  The compiler string comes from the
    !!                    Fortran-2008 intrinsic `compiler_version()` so the binary
    !!                    self-reports which toolchain built it.
    ...
    ! version_stamp is generated by the Makefile (src/fortran/version_stamp.f90,
    ! gitignored) and exposes BUILD_TAG, BUILD_COMMIT and BUILD_DIRTY parameters.
    use version_stamp
    ! compiler_version() returns a deferred-length character describing the
    ! Fortran compiler that produced this binary (e.g. "GCC version 11.4.0").
    ...
    ! Preflight version check: when invoked with a single `--version` argument,
    ! print "<tag> <commit-hash> <clean|dirty> <compiler-string>" on a single
    ! line and exit before any other CLI handling.  The Python wrapper calls
    ! this to confirm the binary was built from the currently-checked-out
    ! source tree and matching toolchain before launching a simulation;
    ! mismatched output triggers a rebuild.
    if (cmd_count == 1) then
        call get_command_argument(1, param, param_len, cmd_status)
        if (cmd_status == 0 .and. param(1:param_len) == '--version') then
            write (*, '(A)') BUILD_TAG // ' ' // BUILD_COMMIT // ' ' // BUILD_DIRTY // ' ' // compiler_version()
            stop
        end if
    end if
