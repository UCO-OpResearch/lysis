program macrolysis
    ...
    !!                  - `--version` flag prints "<commit> <clean|dirty> <compiler>"
    ...
    ! gitignored) and exposes BUILD_COMMIT and BUILD_DIRTY parameters.
    use version_stamp
    ...
    ! print "<commit-hash> <clean|dirty> <compiler-string>" on a single line and
    ! exit before any other CLI handling.  The Python wrapper calls this to
    ! confirm the binary was built from the currently-checked-out source tree
    ! and matching toolchain before launching a simulation; mismatched output
    ! triggers a rebuild.
    if (cmd_count == 1) then
        call get_command_argument(1, param, param_len, cmd_status)
        if (cmd_status == 0 .and. param(1:param_len) == '--version') then
            write (*, '(A)') BUILD_COMMIT // ' ' // BUILD_DIRTY // ' ' // compiler_version()
            stop
        end if
    end if
