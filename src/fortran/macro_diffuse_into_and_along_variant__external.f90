program macrolysis
    ...
    character(:), allocatable   :: inFileCode_big !BB 2026-02-14
    character(:), allocatable   :: inFileCode_small !BB 2026-02-14
...
!!!!!!!!!!!!BB: 2026-02-14!!!!!!!!!!!!!!!!!
    double precision :: frac_forced_big = 0.0852 !value should match result from corresponding micro run, but doesn't matter because Brad's new code will automatically calculate this. This is for bigger Kd (weaker binding)
    double precision :: frac_forced_small = 0.0852 !value should match result from corresponding micro run, but doesn't matter because Brad's new code will automatically calculate this. This is for smaller Kd (stronger binding)
    double precision :: avgwait_big = 2.78 ! measured in seconds, this is the average time a tPA molecule stays bound to fibrin. It's 1/koff, for the bigger Kd (weaker binding)
    double precision :: avgwait_small = 277.8 ! 2.78 ! 27.8 ! measured in seconds, this is the average time a tPA molecule stays bound to fibrin. It's 1/koff, for the smaller Kd (stronger binding)
!!!!!!!!!!!!end BB 2026-02-14!!!!!!!!!!!!
    ...
    integer  :: z
    integer  :: numPLi
    double precision     :: t, avgwait, frac_forced
    ...
    double precision, dimension(101)       :: CDFtPA_big, CDFlys, CDFtPA_small
    double precision, dimension(101)       :: tsec1_big, tseclys, tsec1_small
    ...
    integer, dimension(:), allocatable :: edgetype !vector of 1's and 0's to indicate if the edge tPA is on is partially degraded (1) or fully intact (0)
    ...
    character(40) :: back_row_per_format = '(f23.15)'
    ...
    do while (param_i < cmd_count)
        ...
        case ('inFileCode_big')
            inFileCode_big = param_value(1:param_val_len)
            write (*, *) 'Setting inFileCode_big = ', inFileCode_big
        case ('inFileCode_small')
            inFileCode_small = param_value(1:param_val_len)
            write (*, *) 'Setting inFileCode_small = ', inFileCode_small
        case ('outFileCode')
        ...
        case ('frac_forced_big')
            read (param_value, *, iostat=io_status) frac_forced_big
            if (io_status .ne. 0) then
                write (*, *) 'String conversion error'
                stop
            end if
            write (*, *) 'Setting frac_forced_big = ', frac_forced_big
         case ('frac_forced_small')
            read (param_value, *, iostat=io_status) frac_forced_small
            if (io_status .ne. 0) then
                write (*, *) 'String conversion error'
                stop
            end if
            write (*, *) 'Setting frac_forced_small = ', frac_forced_small
        case ('avgwait_big')
            read (param_value, *, iostat=io_status) avgwait_big
            if (io_status .ne. 0) then
                write (*, *) 'String conversion error'
                stop
            end if
            write (*, *) 'Setting avgwait_big = ', avgwait_big
        case ('avgwait_small')
            read (param_value, *, iostat=io_status) avgwait_small
            if (io_status .ne. 0) then
                write (*, *) 'String conversion error'
                stop
            end if
            write (*, *) 'Setting avgwait_small = ', avgwait_small
        case ('q')
    ...
    allocate (lysismat_big(nummicro, 100))!(100,100) if only did 10,000 micro simulations, (500,100) if did 50,000
    allocate (lysismat_small(nummicro, 100))
    ...
    allocate (edgetype(M)) !vector of 0's and 1's to let me know if the particulat tPA molecule originally bound to intact (0) or partially degraded (1) fibrin
...
!!!!!!!!!!! BB: 2026-02-14. Read in 2 sets of microscale data. !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ! First read in the data for the big Kd (weak binding)
    open (200, file=ADJUSTL('data/'//TRIM(runCode)//'/tPAleave'//TRIM(inFileCode_big)//'.dat'))
    do i = 1, 101
        read (200, *) CDFtPA_big(i)
    end do
    close (200)
    write (*, *) 'read tPAleave_big.dat'

    open (300, file=ADJUSTL('data/'//TRIM(runCode)//'/tsectPA'//TRIM(inFileCode_big)//'.dat'))
    do i = 1, 101
        read (300, *) tsec1_big(i)
    end do
    close (300)
    write (*, *) 'read tsectPA_big.dat'
    ...
    OPEN (unit=201, FILE=ADJUSTL('data/'//TRIM(runCode)//'/lysismat'//TRIM(inFileCode_big)//'.dat'))
    do i = 1, nummicro  ! 100 if only did 10,000 micro simulations, 500 if did 50,000
        READ (201, *) (lysismat_big(i, ii), ii=1, 100)
    end do
    close (201)
    ...
    OPEN (unit=202, FILE=ADJUSTL('data/'//TRIM(runCode)//'/lenlysisvect'//TRIM(inFileCode_big)//'.dat'))
    do i = 1, 100
        READ (202, *) lenlysismat_big(i)
    end do
    close (202)

    ! Then read in data for the small Kd (strong binding)
    open (301, file=ADJUSTL('data/'//TRIM(runCode)//'/tPAleave'//TRIM(inFileCode_small)//'.dat'))
    do i = 1, 101
        read (301, *) CDFtPA_small(i)
    end do
    close (301)
    write (*, *) 'read tPAleave_small.dat'

    open (302, file=ADJUSTL('data/'//TRIM(runCode)//'/tsectPA'//TRIM(inFileCode_small)//'.dat'))
    do i = 1, 101
        read (302, *) tsec1_small(i)
    end do
    close (302)
    write (*, *) 'read tsectPA_small.dat'

    ! lysismat_PLG2_tPA01_Q2.dat is a matrix with column corresponding to bin number (1-100) and with entries
    ! equal to the lysis times obtained in that bin. an entry of 6000 means lysis didn't happen.
    ! lysismat(:,1)=the first column, i.e. the lysis times for the first 100 (or 500 if we did 50,000 micro simulations) tPA leaving times
    OPEN (unit=303, FILE=ADJUSTL('data/'//TRIM(runCode)//'/lysismat'//TRIM(inFileCode_small)//'.dat'))
    do i = 1, nummicro  ! 100 if only did 10,000 micro simulations, 500 if did 50,000
        READ (303, *) (lysismat_small(i, ii), ii=1, 100)
    end do
    close (303)

    ! lenlysisvect_PLG2_tPA01_Q2.dat saves the first row entry in each column of lysismat_PLG2_tPA01_Q2.dat that lysis
    ! did not occur, i.e. the first entry there's a 6000
    OPEN (unit=304, FILE=ADJUSTL('data/'//TRIM(runCode)//'/lenlysisvect'//TRIM(inFileCode_small)//'.dat'))
    do i = 1, 100
        READ (304, *) lenlysismat_small(i)
    end do
    close (304)
!!!!!!!!!!!!!!!!!!END BB 2026-02-14!!!!!!!!!!!!!!!!!!!!
    ...
    open (back_row_per_unit, file=ADJUSTL('data/'//TRIM(runCode)//'/back_row_percent'//TRIM(outFileCode)//'.dat'), form='formatted')
    ...
    do istat = 1, simulations
        ...
        write (*, *) ' kon=', kon
        write (*, *) ' bs=', bs
        write (*, *) ' avgwait_big=', avgwait_big
        write (*, *) ' avgwait_small=', avgwait_small
        write (*, *) ' frac_forced_big=', frac_forced_big
        write (*, *) ' frac_forced_small=', frac_forced_small
        ...
        do
                        ...
                        if (bind(j) .lt. t .and. bind(j) .gt. 0 .and. t_degrade(V(1, j)) >= t) then
                            ...
                            if (t_wait(j) .le. t) then
                               ...
                               if (edgetype(j)==0) then !if the tPA is binding to an intact edge, use the big (weak) Kd
                                ...
                                colr2 = 0
                                do i = 1, 101
                                    if (CDFtPA_big(i) .ge. r3) then
                                        colr2 = i    ! find the first place on CDF that's bigger than r3
                                        exit
                                    end if
                                end do

                                if (colr2 == 0 .or. colr2 == 1) write (*, *) &
                                    'PROBLEM: colr2 should not equal 0 since CDFtPA goes between 0 and 1 exactly', colr2

                                percent2 = (CDFtPA_big(colr2) - r3)/(CDFtPA_big(colr2) - CDFtPA_big(colr2 - 1))
                                ttPA = (tsec1_big(colr2) - (tsec1_big(colr2) - tsec1_big(colr2 - 1))*percent2)
                                ! end if
                                ...
                                if (verbose) then
                                   ...
                                   write (*, '(A, F8.5, A, F8.5)') ' interpolated between ', tsec1_big(colr2), ' and ', tsec1_big(colr2 - 1)
                                   write(*,*)'edgetype = ', edgetype(j)
                                end if
                                ...
                                if (r400 .le. lenlysismat_big(colr2 - 1)) then ! only have lysis if the random number puts us in a bin that's < the first place we have a 6000, i.e. the first place lysis doesn't happen
                                    if (r400 == lenlysismat_big(colr2 - 1)) then
                                        ! percent4 = (CDFlys(r400)-r4)/CDFlys(r400)
                                        rmicro = lysismat_big(r400 - 1, colr2 - 1)! tseclys(r400)-tseclys(r400)*percent4
                                    else
                                        percent4 = r400 - 1 - r4*nummicro
                                        rmicro = (lysismat_big(r400, colr2 - 1) - (lysismat_big(r400, colr2 - 1) &
                                                                            - lysismat_big(r400 - 1, colr2 - 1))*percent4)
                                    end if
                                    ...
                                    !     t_degrade(V(1,j)) = min(t_degrade(V(1,j)),(t+rmicro-tstep/2)) ! choose the smallest time, because that's what will happen first
                                    ! end if

                                end if ! for if(r400.le.lenlysismat) loop
                               else !for if (edgetype...). if tPA is binding to partially degraded fibrin, use small Kd (strong binding)
                                ! find the time that tPA will unbind:
                                colr2 = 0
                                do i = 1, 101
                                  if (CDFtPA_small(i) .ge. r3) then
                                    colr2 = i    ! find the first place on CDF that's bigger than r3
                                    exit
                                  end if
                               end do
                               ...
                               if (colr2 == 0 .or. colr2 == 1) write (*, *) &
                                 ...
                                 percent2 = (CDFtPA_small(colr2) - r3)/(CDFtPA_small(colr2) - CDFtPA_small(colr2 - 1))
                                 ttPA = (tsec1_small(colr2) - (tsec1_small(colr2) - tsec1_small(colr2 - 1))*percent2)
                               ! end if
                                ...
                                if (verbose) then
                                  ...
                                  write (*, '(A,I10,A,I5,A,F8.5,A,F5.4)') '-> ts ', count - 1, &
                                       ' -> m ', j - 1, ' leaving time set to ', t_leave(j), '; using r = ', r3
                                  write (*, '(A, F8.5, A, F8.5)') ' interpolated between ', tsec1_small(colr2), ' and ', tsec1_small(colr2 - 1)
                                  write(*,*)'edgetype = ', edgetype(j)
                                end if
                               ...
                               if (r400 .le. lenlysismat_small(colr2 - 1)) then ! only have lysis if the random number puts us in a bin that's < the first place we have a 6000, i.e. the first place lysis doesn't happen
                                   if (r400 == lenlysismat_small(colr2 - 1)) then
                                   ! percent4 = (CDFlys(r400)-r4)/CDFlys(r400)
                                       rmicro = lysismat_small(r400 - 1, colr2 - 1)! tseclys(r400)-tseclys(r400)*percent4
                                   else
                                       percent4 = r400 - 1 - r4*nummicro
                                       rmicro = (lysismat_small(r400, colr2 - 1) - (lysismat_small(r400, colr2 - 1) &
                                            - lysismat_small(r400 - 1, colr2 - 1))*percent4)
                                   end if
                            ...
                            if (r2 .gt. (t - bind(j))/tstep) then   ! if r2 is such that movement happened before binding, move the molecule
                             ...
                             ! find the time that tPA will unbind:
                                ...
                                if (verbose) then
                                    ...
                                    write (*, '(A, I10, A, I5, A, F8.5, A, F5.4)') '-> ts ', count - 1, &
                                        ' -> m ', j - 1, ' leaving time set to ', t_leave(j), '; using r = ', r3
                                    write (*, '(F8.5, A, F8.5, A, F8.5)') &
                                        ttPA, '; interpolated between ', tsec1_big(colr2), ' and ', tsec1_big(colr2 - 1)
                                    write (*,*) 'edgetype = ', edgetype(j)
            ...
            if (Ninteger >= Nsave) then ! if the current time is the 1st past a new a 10 seconds, e.g. t=10.001, save degrade and V
                ...
                write(back_row_per_unit,back_row_per_format) reached_back_row_percent
