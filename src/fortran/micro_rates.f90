...
double precision  :: kplgoff
double precision  :: kaoff12
double precision  :: kaoff10
double precision  :: kapcat
double precision  :: kplgoffnick
double precision  :: kplioff
double precision  :: kcat
double precision  :: kncat
...
do stats = 1,runs
  ...
  if(stats==1) then
    write(lysfile,'(58a)'  ) 'lysisPLG2_tPA01_Q2.dat'
    write(tPAfile,'(61a)'  ) 'tPA_timePLG2_tPA01_Q2.dat'
    write(PLifile,'(56a)' ) 'PLiPLG2_tPA01_Q2.dat'
    write(endfile,'(64a)' ) 'lyscompletePLG2_tPA01_Q2.dat'
    write(plgfile,'(56a)' ) 'PLGPLG2_tPA01_Q2.dat'
    write(ctfile,'(58a)' ) 'countPLG2_tPA01_Q2.dat'
    !write(plgbdfile,'(27a)') 'PLGunbindPLG2_tPA01_Q2.dat' 
    !write(plgunbdfile,'(29a)') 'PLGbindPLG2_tPA01_Q2.dat' 
    write(plitimefile,'(63a)') 'PLitimePLG2_tPA01_Q2.dat'
    write(tPAPLifile,'(63a)') 'tPAPLiunbdPLG2_tPA01_Q2.dat'
    !write(sfile,'(28a)' ) 'statetPAPLG2_tPA01_Q2.dat'
    !write(profile,'(23a)' ) 'persavePLG2_tPA01_Q2.dat'
    !write(t2file,'(23a)' ) 'tsavePLG2_tPA01_Q2.dat'
    write(tPAunbdfile,'(61a)') 'tPAunbindPLG2_tPA01_Q2.dat'
    write(s2file,'(67a)' ) 'lasttPAPLG2_tPA01_Q2.dat'
    write(fpfile,'(77a)') 'firstPLiPLG2_tPA01_Q2.dat'
    ...
    write(*,*)' kaoff10=',param(8,ipar)
    write(*,*)' kaoff12=',param(7,ipar)
    write(*,*)' kplioff=',param(6,ipar)
    write(*,*)' kplgoffnick=',param(5,ipar)
    write(*,*)' kplgon=',param(4,ipar)
    write(*,*)' kplgoff=',param(3,ipar)
    write(*,*)' freeplg=',param(2,ipar)
    write(*,*)' kdeg=',param(1,ipar)
...
!set the reaction rates to be the appropriate parameters. I think we likely don't need this step in future versions of the code; we could directly use the parameters, e.g. "kplgon*KdPLGintact" to represent the unbinding rate of PLG
   ...
   c5 = param(6,ipar)
   c6 = param(7,ipar)
   c8 = param(4,ipar)*param(2,ipar)
   c8a = param(5,ipar)!*param(2,ipar)
   c9 = param(3,ipar)
   c11 = param(6,ipar)
   c12 = param(7,ipar)
   c13 = param(8,ipar)
...
!
    ...
    if(tPA(count-1)==1) then
      ...
      do i=1,Ntot
         do j=1,nodes**2
            if(state(i,j).eq.2.or.state(i,j).eq.6)then
               countstate13=countstate13+1
            end if
         enddo
      enddo
      ...
      countstate14=0
         do j=1,nodes**2
            if(state(1,j).eq.1)then
               countstate14=countstate14+1
            end if
         enddo
