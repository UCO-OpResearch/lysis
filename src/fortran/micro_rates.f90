program micromodel
    ...
    ! print "<commit-hash> <clean|dirty>" on a single line and exit before
    ! any other CLI handling.  The Python wrapper calls this to confirm
    ! the binary was built from the currently-checked-out source tree
    ! before launching a simulation; mismatched output triggers a rebuild.
    if (cmd_count == 1) then
