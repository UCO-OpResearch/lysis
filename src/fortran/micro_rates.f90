program micromodel
    ...
    ! print "<commit-hash> <clean|dirty> <compiler-string>" on a single line
    ! and exit before any other CLI handling.  The Python wrapper calls this
    ! to confirm the binary was built from the currently-checked-out source
    ! tree and matching toolchain before launching a simulation; mismatched
    ! output triggers a rebuild.
    if (cmd_count == 1) then
