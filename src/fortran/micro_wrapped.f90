program micromodel
    ...
    ! integer  :: countstate6, countstate7, countstate8,
    integer  :: countstate9, countstate10
    ! integer  :: countlat2,
    integer  :: countlat3
    integer  :: countstate11, countstate12, countstate13, countstate14, countstate15, countstate10a
    ! integer  :: countstate11a
    ...
    external set_kiss32, get_kiss32
    ...
    integer  :: kiss32, mscw, stater(4)
    ! integer  :: old_stater(4)
    integer  :: ui
    ...
    do stats = 1, runs
        ...
        do
            ...
            if (bigJ(place) == 14) then
                ...
                call movetpa(Ntot, nodes, state, prob_N02i, prob_N00i, prob_N02n, prob_N00n, vol3, ktPAon, D, p_rebind)
            ...
            elseif (bigJ(place) == 17) then
                ...
                do iPLi2 = 1, countPLi
                    if (PLilocation(1, iPLi2) == PLilocsmall(1) .and. PLilocation(2, iPLi2) == PLilocsmall(2) &
                        .and. iPLi2 /= loc) then
                        loc2b = iPLi2
                        if (loc2b /= loc2) then
            ...
            elseif (bigJ(place) == 18 .or. bigJ(place) == 19 .or. bigJ(place) == 20 &
                    .or. bigJ(place) == 21 .or. bigJ(place) == 22) then
            ...
            elseif (bigJ(place) == 3 .or. bigJ(place) == 7 .or. bigJ(place) == 10 &
                    .or. bigJ(place) == 11 .or. bigJ(place) == 13) then
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    subroutine vurcw1(v_sub, n)
        double precision v_sub(*)
        integer k, n
...
!
    subroutine findint(g, sizeg, intspec, sizegnew)
        integer  :: sizeg, sizegnew
        integer  :: i_sub, countg, intspec
        integer, dimension(sizeg)   :: g

        countg = 0

        do i_sub = 1, sizeg
            if (g(i_sub) == intspec) then
                countg = countg + 1
            end if
...
!
    subroutine findfirst(g, sizeg, numspec, loca_sub)
        integer  :: sizeg
        integer  :: i_sub, numspec, loca_sub
        integer, dimension(sizeg)   :: g

        loca_sub = 0
        do i_sub = 1, sizeg
            if (g(i_sub) == numspec) then
                loca_sub = i_sub
                exit
            end if
...
!
    ...
    subroutine randchoice(g, sizeg, numspec, loca_sub)

        integer  :: sizeg

        integer  :: i_sub, numspec, loca_sub, count_sub, sizel

        integer, dimension(sizeg)   :: g
        double precision :: r_sub
        ...
        sizel = count_sub
        if (sizel == 1) then
            loca_sub = location(count_sub)
        elseif (sizel == 0) then
            loca_sub = 0
            write (*, *) 'Problem:should only access subroutine if at least one entry is this number'
        else
            do i_sub = 1, sizel
                prob(i_sub) = 1.0/sizel  !have it be equally likely to choose any entry
            end do
            ...
            do i_sub = 1, sizel
                if (r_sub <= cumprob(i_sub)) then
                    loca_sub = location(i_sub)
                    exit
                end if
            end do
        end if
...
!
    subroutine cumsum(g, sizeg, gnew)
        ...
        do i_sub = 1, sizeg
            currentsum = currentsum + g(i_sub)
            gnew(i_sub) = currentsum
        end do
...
!
    subroutine movetpa(Ntot, nodes, state, prob_N02i, prob_N00i, prob_N02n, prob_N00n, vol3, param9, D, p_rebind)
