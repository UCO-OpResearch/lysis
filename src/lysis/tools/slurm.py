...
# ===========================================================================
# Macroscale Slurm helpers
# ===========================================================================

#: Template for the macroscale master Python script.
_MACRO_MASTER_PY_TEMPLATE = Template('''\
#!/usr/bin/env python3
"""Master Slurm job: submit child Fortran macro jobs, poll for completion, import results."""
import time
import shutil
from pathlib import Path
...
def _generate_macro_master_py(
    ...
    """Return the content of the macroscale master Python script.
...
def generate_macro_child_script(
    staging_dir: "Path | str",
    run_code: str,
    hdf5_path: "Path | str",
    executable: "Path | str",
    ...
    *,
    ...
    """Generate a Slurm bash script for a single child macroscale Fortran job.

    The child script sources the user's shell environment, copies the
    executable into the working directory, and calls
    :meth:`~lysis.execution.codeutil.FortranMacro.exec_in_workdir` via
    ``python -c``.
...
from pathlib import Path
from lysis.execution.codeutil import FortranMacro
fm = FortranMacro.from_hdf5('{hdf5_path}', '{staging_dir}/{binary_name}', out_file_code='{out_code}')
fm.exec_in_workdir(Path('{staging_dir}'), Path('{hdf5_path}'))
" """
...
from pathlib import Path
from lysis.execution.codeutil import FortranMacro
fm = FortranMacro.from_hdf5('{hdf5_path}', '${{local_work_dir}}/{binary_name}', out_file_code='{out_code}')
fm.exec_in_workdir(Path('${{local_work_dir}}'), Path('{hdf5_path}'))
" """
...
def submit_macro_slurm_job(
    ...
    *,
    ...
    # Write child script
    # ------------------------------------------------------------------
    child_script = generate_macro_child_script(
        staging_dir, run_code, hdf5_path, executable, out_code,
        partition=partition, fast_tmp_root=fast_tmp_root,
    )
    child_path = staging_dir / "child_000.sh"
