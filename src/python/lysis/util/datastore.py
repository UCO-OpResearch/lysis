...
_data_post_load = {
    ...
    # At the end of finite data, the rest of the column is filled with the
    # value 6000 to indicate null or infinite.
    # Thus, we trim the lysis_time matrix to eliminate most of the rows that
    # contain just 6000. For now, I'm hardcoding this to keep 110 rows.
    # The actual amount of data that needs to be kept is total_lyses.max()
    #
    # Finally we transpose the matrix for convenient access in Python
    # (e.g. matrix[i] = row)
...
class DataStore:
    ...
    def __setattr__(self, key: AnyStr, value: Any):
        if key[0] == "_":
            ...
            raise RuntimeError(
                f"{key} already exists (as {self._filenames[key]}). "
    ...
    def _set_status(self, key: AnyStr, status: DataStatus):
        ...
        if current_status == DataStatus.NONE:
            raise RuntimeError(
                f"This shouldn't happen. Don't try to set the status without at "
                f"least a filename set."
            )
    ...
    def new(self, key: AnyStr, filename: AnyStr):
        ...
        if key in self._internal_names:
            ...
            if DataStatus.INITIALIZED not in self.status(key[3:]):
                raise RuntimeError(
                    f"'{key}' is already initialized with file "
                    f"'{self._filenames[key[3:]]}'."
                )
            else:
            ...
            raise RuntimeError(
                f"'{key}' is already initialized with file '{self._filenames[key]}'."
            )
        else:
    ...
    def delete(self, key: AnyStr):
        if DataStatus.SAVED in self.status(key):
            raise RuntimeError(
                f"'{key}' already saved to disk as '{self._filenames[key]}'. '"
                f"This module should NOT be used for modifying existing data on disk."
            )
    ...
    def append(self, key: AnyStr, value: Any, axis: int | None = None):
        if key[:3] == "cp_":
            if DataStatus.INITIALIZED not in self.status(key[3:]):
                ...
                raise RuntimeError(
                    f"'{key}' already saved to disk as '{self._filenames[key[3:]]}'. '"
                    f"This module should NOT be used for modifying existing data "
                    f"on disk."
                )
            elif DataStatus.LOADED in self.status(key):
    ...
    def load_from_disk(self, key: AnyStr):
        ...
        if key[:3] == "cp_":
            if DataStatus.INITIALIZED not in self.status(key[3:]):
                ...
                raise RuntimeError(
                    f"No file for {key} found on disk. "
        ...
        # TODO(bpaynter): Implement, consistent with the rest of the model
            ...
            self._data[key] = xp.array(post_load(data))
        # Other file types/extensions are not supported (or planned to be) at
        # this time
        else:
            raise AttributeError("Non-supported file type.")

    def save_to_disk(self, key: AnyStr):
        # TODO(bpaynter): It needs to be checked whether the text files output
        #   by this method can be read by the existing Fortran and Matlab code.
        filename = None
        pre_save = identity
        if key[:3] == "cp_":
            if DataStatus.INITIALIZED not in self.status(key[3:]):
        ...
        if os.path.splitext(filename)[1] == ".dat":
            ...
            np.savetxt(
                ...
                pre_save(self._data[key]),
                newline=os.linesep,
            )
        else:
            # Save as a NumPy file with the extension .npy
            np.save(os.path.join(self._path, filename), pre_save(self._data[key]))
